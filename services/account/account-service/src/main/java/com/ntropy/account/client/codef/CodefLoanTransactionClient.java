...
/** CODEF 은행 개인 대출 거래내역 API 클라이언트. */
@Component
@RequiredArgsConstructor
public class CodefLoanTransactionClient {

    private static final String PATH = "/v1/kr/bank/p/loan/transaction-list";
    ...
    public JsonNode getPersonalTransactionList(String organizationCode, String connectedId, String account,
                                               String accountLoanExecNo, LocalDate startDate, LocalDate endDate,
                                               String birthDate) {
        ...
        if (accountLoanExecNo != null && !accountLoanExecNo.isBlank()) {
            requestBody.put("accountLoanExecNo", accountLoanExecNo);
        }
        if (birthDate != null) {
        ...
        if (!"CF-00000".equals(resultCode)) {
            throw new IllegalStateException(
                    "CODEF 개인 대출 거래내역 조회 실패: " + CodefErrorMessage.from(response)
            );
        }
        JsonNode data = response.path("data");
        if (!data.isObject() && !data.isArray()) {
            throw new IllegalStateException("CODEF 개인 대출 거래내역 응답의 data 형식을 알 수 없습니다");
        }
        return response;
    }
}
